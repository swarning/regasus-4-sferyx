...
public class FileCacheStatisticsComposite extends Composite {

	// the entity
	private FileCacheStatistics cacheStatistics;
	...
	public FileCacheStatisticsComposite(Composite parent, int style) throws Exception {
		super(parent, style);
	...
	private void createWidgets() throws Exception {
		...
		SWTHelper.createLabel(this, "Cache Size:");
		cacheSizeLabel = new Label(this, SWT.NONE);
		widgetGridDataFactory.applyTo(cacheSizeLabel);
	...
	private void createTable(Composite parent) {
		...
		layout.setColumnData(internalPathTableColumn, new ColumnWeightData(80));

		TableColumn sizeTableColumn = new TableColumn(table, SWT.RIGHT);
		sizeTableColumn.setText("Size");
		layout.setColumnData(sizeTableColumn, new ColumnWeightData(10));
		...
		FileCacheStatisticsRecordTable fileStatisticsRecordTable = new FileCacheStatisticsRecordTable(table);
		tableViewer = fileStatisticsRecordTable.getViewer();
	}


	public void setCacheStatistics(FileCacheStatistics cacheStatistics) {
			...
			SWTHelper.syncExecDisplayThread(new Runnable() {
				...
				public void run() {
					try {
						...
						long totalSize = cacheStatistics.getTotalSize();
						cacheSizeLabel.setText( FileHelper.computeReadableFileSize(totalSize) );
						...
						tableViewer.setInput( cacheStatistics.getFileStatisticsRecordList() );
