...
import de.regasus.impex.ods.dialog.ODSParticipantImportWizard;
import de.regasus.impex.ui.Activator;


public class ODSParticipantImportHandler extends AbstractHandler {

	public Object execute(ExecutionEvent event) throws ExecutionException {
		if (PlatformUI.getWorkbench().saveAllEditors(true)) {
			try {
				ODSParticipantImportWizard wizard = new ODSParticipantImportWizard();
				
				// init wizard with selected event
				List<Long> eventIDs = EventSelectionHelper.getEventIDs(event);
				if (eventIDs != null && eventIDs.size() == 1) {
					ODSParticipantImportWizard.setEventPK(eventIDs.get(0));
				}
