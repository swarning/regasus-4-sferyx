...
public class CostCenterView extends AbstractView {
	public static final String ID = "CostCenterView";

	// Widgets
	private CostCenterTableComposite costCenter1TableComposite;
	...
	protected boolean isVisible() {
		...
		if (getConfigParameterSet() != null) {
			visible = getConfigParameterSet().getInvoiceDetails().getCostCenter1().isVisible();
		}
		return visible;
	}
	...
	public void createWidgets(Composite parent) throws Exception {
		if (isVisible()) {
			costCenter1TableComposite = new CostCenterTableComposite(parent, SWT.FULL_SELECTION | SWT.MULTI);

			// make the Tree the SelectionProvider
			TableViewer tableViewer = costCenter1TableComposite.getTableViewer();
			...
			// setTitleToolTip(FinanceI18N.CostCenterView_ToolTip);
	...
	private void initializeActions() {
		if (actionList == null) {
			...
			actionList.add(new CreateCostCenterAction(window));

			editAction = new EditCostCenterAction(window);
			actionList.add(editAction);

			actionList.add(new DeleteCostCenterAction(window));

			actionList.add(new RefreshCostCenterAction());
	...
	private void initializeContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			...
			public void menuAboutToShow(IMenuManager manager) {
				CostCenterView.this.fillContextMenu(manager);
			}
		});
	...
	private void initializeDoubleClickAction() {
		final TableViewer tableViewer = costCenter1TableComposite.getTableViewer();
	...
	public void setFocus() {
		try {
			if (costCenter1TableComposite != null &&
				!costCenter1TableComposite.isDisposed() &&
				costCenter1TableComposite.isEnabled()
			) {
				costCenter1TableComposite.setFocus();
			}
		}
		catch (Exception e) {
