...
public class CostCenterEditor 
extends AbstractEditor<CostCenterEditorInput>
implements IRefreshableEditorPart, CacheModelListener<Integer> {

	public static final String ID = "CostCenterEditor"; 
	...
	protected void init() throws Exception {
		...
		if (key != null) {
			...
			// get entity
			costCenterVO = costCenter1Model.getCostCenterVO(key);
						
			// register at model
			costCenter1Model.addListener(this, key);
		}
		else {
			// create empty entity
	...
	@Override
	public void dispose() {
		if (costCenter1Model != null && costCenterVO.getPK() != null) {
			try {
				costCenter1Model.removeListener(this, costCenterVO.getPK());
			}
			catch (Exception e) {
	...
	protected String getTypeName() {
		return InvoiceLabel.CostCenter.getString();
	}
...
//	}
	...
	public void doSave(IProgressMonitor monitor) {
		...
		try {
			...
			if (create) {
				 ...
				 */
				costCenterVO = costCenter1Model.create(costCenterVO);

				create = false;
				
				// observe the CountryModel
				costCenter1Model.addListener(this, costCenterVO.getPK());
				 ...
				 */
				costCenter1Model.update(costCenterVO);
			}
	...
	public void dataChange(CacheModelEvent<Integer> event) {
		try {
			if (event.getSource() == costCenter1Model) {
				if (event.getOperation() == CacheModelOperation.DELETE) {
					closeBecauseDeletion();
				}
				else if (costCenterVO != null) {
					costCenterVO = costCenter1Model.getCostCenterVO(costCenterVO.getPK());
					if (costCenterVO != null) {
						setEntity(costCenterVO);
					}
	...
	public void refresh() throws Exception {
		if (costCenterVO != null && costCenterVO.getPK() != null) {
			costCenter1Model.refresh(costCenterVO.getPK());
	...
	protected String getName() {
		...
		if (costCenterVO.getNo() != null) {
			...
			name = FinanceI18N.CostCenter_Editor_NewName;
		}
	...
	protected String getToolTipText() {
		return FinanceI18N.CostCenter_Editor_DefaultToolTip;
	}
	...
	protected void openInfoDialog() {
		...
		final EditorInfoDialog infoDialog = new EditorInfoDialog(
			getSite().getShell(),
			InvoiceLabel.CostCenter.getString() + ": " + UtilI18N.Info,
			labels,
			values
		);
		infoDialog.open();
