...
public class IncreaseReminderLevelCommandHandler extends AbstractInvoiceCommandHandler {
	...
	public void execute(ExecutionEvent event, Collection<InvoiceVO> invoiceVOs) throws Exception {
		try {
			...
			if (applicableCount > 0) {
				...
				// Ask the user whether to increase the reminder level for the selected invoices
				boolean confirmed = MessageDialog.openConfirm(
					shell,
					UtilI18N.Question,
					I18N.IncreaseReminderStateQuestion + getApplicableCountDetail(selectedCount, applicableCount)
				);
				if (confirmed) {
	...
	public static boolean isApplicable(InvoiceVO invoiceVO) {
		boolean applicable =
			...
			invoiceVO.getReminderState() != ReminderState.PAID &&
			invoiceVO.getReminderState() != ReminderState.LEVEL5 &&
	...
	private void executeInChunks (
		...
		ChunkExecutor<InvoiceVO> chunkExecutor = new ChunkExecutor<InvoiceVO>() {
			...
			protected void executeChunk(List<InvoiceVO> chunkList) throws Exception {
				// increase the remind state
				AccountancyModel.getInstance().remindInvoices(chunkList);
		...
		String operationMessage = I18N.IncreaseReminderLevelCommandHandler_OperationMessage;
		...
		SWTHelper.syncExecDisplayThread(new Runnable() {
			public void run() {
				try{						
					...
					String title = I18N.IncreaseReminderLevelCommandHandler_Title;
					String message = I18N.IncreaseReminderLevelCommandHandler_FinalMessage;
