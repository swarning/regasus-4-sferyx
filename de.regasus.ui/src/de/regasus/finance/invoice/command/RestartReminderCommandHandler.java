...
public class RestartReminderCommandHandler extends AbstractInvoiceCommandHandler {
	...
	public void execute(ExecutionEvent event, Collection<InvoiceVO> invoiceVOs) throws Exception {
		try {
			...
			if (applicableCount > 0) {
				...
				// Ask the user whether to restart the reminder process for the selected invoices
				boolean confirmed = MessageDialog.openConfirm(
					shell,
					UtilI18N.Question,
					I18N.RestartReminderQuestion + getApplicableCountDetail(selectedCount, applicableCount)
				);
	...
	private void executeInChunks (
		...
		ChunkExecutor<InvoiceVO> chunkExecutor = new ChunkExecutor<InvoiceVO>() {
			...
			protected void executeChunk(List<InvoiceVO> chunkList) throws Exception {
				// restart the remind state
				AccountancyModel.getInstance().restartReminder(chunkList);
		...
		String operationMessage = I18N.RestartReminderLevelCommandHandler_OperationMessage;
		...
		SWTHelper.syncExecDisplayThread(new Runnable() {
			public void run() {
				try{						
					...
					String title = I18N.RestartReminderLevelCommandHandler_Title;
					String message = I18N.RestartReminderLevelCommandHandler_FinalMessage;
