...
public class CostUnitView extends AbstractView {
	public static final String ID = "CostUnitView";

	// Widgets
	private CostUnitTableComposite costUnitTableComposite;
	...
	protected boolean isVisible() {
		...
		if (getConfigParameterSet() != null) {
			visible = getConfigParameterSet().getInvoiceDetails().getCostCenter2().isVisible();
		}
		return visible;
	}
	...
	public void createWidgets(Composite parent) throws Exception {
		if (isVisible()) {
			costUnitTableComposite = new CostUnitTableComposite(parent, SWT.FULL_SELECTION | SWT.MULTI);
			// make the Tree the SelectionProvider
			TableViewer tableViewer = costUnitTableComposite.getTableViewer();
			...
			// setTitleToolTip(FinanceI18N.CostUnitView_ToolTip);
	...
	private void initializeActions() {
		if (actionList == null) {
			...
			actionList.add(new CreateCostUnitAction(window));

			editAction = new EditCostUnitAction(window);
			actionList.add(editAction);

			actionList.add(new DeleteCostUnitAction(window));

			actionList.add(new RefreshCostUnitAction());
	...
	private void initializeContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			...
			public void menuAboutToShow(IMenuManager manager) {
				CostUnitView.this.fillContextMenu(manager);
			}
		});

		final TableViewer tableViewer = costUnitTableComposite.getTableViewer();
	...
	public void setFocus() {
		try {
			if (costUnitTableComposite != null &&
				!costUnitTableComposite.isDisposed() &&
				costUnitTableComposite.isEnabled()
			) {
				costUnitTableComposite.setFocus();
			}
		}
		catch (Exception e) {
