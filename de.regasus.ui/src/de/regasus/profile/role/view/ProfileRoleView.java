...
public class ProfileRoleView extends AbstractView {

	public static final String ID = "ProfileRoleView";

	// Widgets
	private ProfileRoleTableComposite profileRoleTableComposite;
	...
	protected boolean isVisible() {
		...
		if (getConfigParameterSet() != null) {
			visible = 	getConfigParameterSet().getProfile().isVisible() &&
						getConfigParameterSet().getProfile().getProfileRole().isVisible();
		}
		return visible;
	}
	...
	public void createWidgets(Composite parent) throws Exception {
		if (isVisible()) {
			profileRoleTableComposite = new ProfileRoleTableComposite(
				parent,
				SWT.FULL_SELECTION | SWT.MULTI
			);

			// make the Tree the SelectionProvider
			TableViewer tableViewer = profileRoleTableComposite.getTableViewer();
			...
			setTitleToolTip(I18N.ProfileRoleView_ToolTip);
	...
	private void initializeActions() {
		if (actionList == null) {
			...
			actionList.add(new CreateProfileRoleAction(window));

			editAction = new EditProfileRoleAction(window);
			actionList.add(editAction);

			actionList.add(new DeleteProfileRoleAction(window));

			actionList.add(new RefreshProfileRoleAction());
	...
	private void initializeContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			...
			public void menuAboutToShow(IMenuManager menuManager) {
				ProfileRoleView.this.fillContextMenu(menuManager);
			}
		});
	...
	private void initializeDoubleClickAction() {
		final TableViewer tableViewer = profileRoleTableComposite.getTableViewer();
	...
	public void setFocus() {
		 ...
		 */
		try {
			if (profileRoleTableComposite != null
				&& !profileRoleTableComposite.isDisposed()
				&& profileRoleTableComposite.isEnabled()
			) {
				profileRoleTableComposite.setFocus();
			}
		}
		catch (Exception e) {
