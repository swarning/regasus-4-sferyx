...
public class EsraMembershipComponentComposite extends EntityComposite<EsraMembershipComponent> {
	...
	private I18NComposite<EsraMembershipComponent> i18nComposite;
	...
	public EsraMembershipComponentComposite(Composite parent, int style, Long portalPK)
	throws Exception {
		super(
			parent,
			style,
	...
	protected void createWidgets(Composite parent) throws Exception {
		...
		widgetBuilder.buildTypeLabel( PortalI18N.EsraMembershipComponent.getString() );
	...
	private void buildI18NWidgets(Composite parent) {
		i18nComposite = new I18NComposite<>(parent, SWT.BORDER, languageList, new EsraMembershipI18NWidgetController());
	...
	private void buildNonMemberHint(Composite parent) {
		...
		label.setText( EsraMembershipComponent.NON_MEMBER_HINT.getString() );
	...
	private void buildUnconfimedMembershipMessage(Composite parent) {
		Label label = new Label(parent, SWT.RIGHT);
		GridDataFactory.swtDefaults().align(SWT.LEFT, SWT.CENTER).span(COL_COUNT, 1).indent(SWT.DEFAULT, 10).applyTo(label);
		label.setText( EsraMembershipComponent.UNCONFIRMED_MEMBERSHIP_MESSAGE.getString() );
		SWTHelper.makeBold(label);
