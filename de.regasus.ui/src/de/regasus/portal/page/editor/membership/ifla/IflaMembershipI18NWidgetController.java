...
public class IflaMembershipI18NWidgetController implements I18NWidgetController<IflaMembershipComponent>{


	// the entity
	private IflaMembershipComponent membershipComponent;
	...
	private Map<String, Text> enterMembershipNumberLabelWidgetMap = new HashMap<>();
	private Map<String, Text> membershipNumberLabelWidgetMap = new HashMap<>();


	public IflaMembershipI18NWidgetController() {
	}
	...
	public void createWidgets(Composite parent, ModifySupport modifySupport, String lang) {
		...
		membershipLabelWidgetMap.put(
			lang,
			builder
				.fieldMetadata(IflaMembershipComponent.MEMBERSHIP_LABEL)
				.bold(true)
				.build()
		);
		...
		memberButtonLabelWidgetMap.put(
			lang,
			builder
				.fieldMetadata(IflaMembershipComponent.MEMBER_BUTTON_LABEL)
				.bold(true)
				.build()
		);
		...
		nonMemberButtonLabelWidgetMap.put(
			lang,
			builder
				.fieldMetadata(IflaMembershipComponent.NON_MEMBER_BUTTON_LABEL)
				.bold(true)
				.build()
		);

		enterMembershipNumberLabelWidgetMap.put(
			lang,
			builder
				.fieldMetadata(IflaMembershipComponent.ENTER_MEMBERSHIP_NUMBER_LABEL)
				.bold(true)
				.build()
		);
		...
		membershipNumberLabelWidgetMap.put(
			lang,
			builder
				.fieldMetadata(IflaMembershipComponent.MEMBERSHIP_NUMBER_LABEL)
				.bold(true)
				.build()
		);
	}
	...
	@Override
	public IflaMembershipComponent getEntity() {
		return membershipComponent;
	}


	@Override
	public void setEntity(IflaMembershipComponent membershipComponent) {
			...
			SWTHelper.syncExecDisplayThread(new Runnable() {
				...
				public void run() {
					try {
						...
						setLanguageStringToTextWidget(membershipComponent.getEnterMembershipNumberLabel(), enterMembershipNumberLabelWidgetMap);
	...
	public void syncEntityToWidgets() {
		if (membershipComponent != null) {
			...
			membershipComponent.setEnterMembershipNumberLabel( buildLanguageString(enterMembershipNumberLabelWidgetMap) );
