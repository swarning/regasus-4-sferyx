...
public class EcfsMembershipI18NWidgetController implements I18NWidgetController<EcfsMembershipComponent>{


	// the entity
	private EcfsMembershipComponent membershipComponent;

	// widget Maps
	private Map<String, Text> membershipLabelWidgetMap = new HashMap<>();
	private Map<String, Text> membershipQuestionLabelWidgetMap = new HashMap<>();
	...
	public EcfsMembershipI18NWidgetController() {
	}
	...
	public void createWidgets(Composite parent, ModifySupport modifySupport, String lang) {
		...
		membershipLabelWidgetMap.put(
			lang,
			builder
				.fieldMetadata(EcfsMembershipComponent.MEMBERSHIP_LABEL)
				.bold(true)
				.build()
		);

		membershipQuestionLabelWidgetMap.put(
			lang,
			builder
				.fieldMetadata(EcfsMembershipComponent.MEMBERSHIP_QUESTION_LABEL)
				.bold(true)
				.build()
		);
		...
		memberButtonLabelWidgetMap.put(
			lang,
			builder
				.fieldMetadata(EcfsMembershipComponent.MEMBER_BUTTON_LABEL)
				.bold(true)
				.build()
		);
		...
		nonMemberButtonLabelWidgetMap.put(
			lang,
			builder
				.fieldMetadata(EcfsMembershipComponent.NON_MEMBER_BUTTON_LABEL)
				.bold(true)
				.build()
		);
		...
		membershipNumberLabelWidgetMap.put(
			lang,
			builder
				.fieldMetadata(EcfsMembershipComponent.MEMBERSHIP_NUMBER_LABEL)
				.bold(true)
				.build()
		);
	}
	...
	@Override
	public EcfsMembershipComponent getEntity() {
		return membershipComponent;
	}


	@Override
	public void setEntity(EcfsMembershipComponent membershipComponent) {
			...
			SWTHelper.syncExecDisplayThread(new Runnable() {
				...
				public void run() {
					try {
						...
						setLanguageStringToTextWidget(membershipComponent.getMembershipQuestionLabel(), membershipQuestionLabelWidgetMap);
	...
	public void syncEntityToWidgets() {
		if (membershipComponent != null) {
			...
			membershipComponent.setMembershipQuestionLabel( buildLanguageString(membershipQuestionLabelWidgetMap));
