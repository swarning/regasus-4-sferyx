...
public class OpenIDConnectStyleComposite extends Composite implements StyleParameterComposite {

	private static final String OPEN_ID_CONNECT_LESS_RESOURCES_FILE_NAME = "openid-less-resources.zip";
	...
	private OpenIDStyleColorComposite colorComposite;
	private OpenIDStyleFontComposite fontComposite;
	private OpenIDStyleSizeComposite sizeComposite;
	...
	public OpenIDConnectStyleComposite(Composite parent) {
	...
	public void createWidgets() throws Exception {
		...
		// Color Tab
		{
    		...
    		colorComposite = new OpenIDStyleColorComposite(tabFolder);
		...
		// Font Tab
		{
			...
			fontComposite = new OpenIDStyleFontComposite(tabFolder);
		...
		// Size Tab
		{
			...
			sizeComposite = new OpenIDStyleSizeComposite(tabFolder);
...
//					directory = new File("/Users/sacha/tmp"); // comment-in for debug
		    		...
		    		if ( ! applicationLessFile.exists()) {
		    			throw new RuntimeException(OPEN_ID_CONNECT_LESS_RESOURCES_FILE_NAME + " does not contain the expected file application.less.");
		    		}
...
//		}
	...
	private void copyLessResources(File dir) throws ClassNotFoundException, IOException {
		...
		if (styleTemplateClass != null) {
			InputStream inputStream = styleTemplateClass.getResourceAsStream(OPEN_ID_CONNECT_LESS_RESOURCES_FILE_NAME);
			if (inputStream != null) {
