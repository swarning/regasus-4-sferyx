...
public class StandardStyleComposite extends Composite implements StyleParameterComposite {

	protected static final String STANDARD_LESS_RESOURCES_FILE_NAME = "standard-less-resources.zip";
	protected static final String CLASSIC_LESS_RESOURCES_FILE_NAME = "classic-less-resources.zip";
	...
	private StandardStyleColorComposite colorComposite;
	private StandardStyleFontComposite fontComposite;
	private StandardStyleSizeComposite sizeComposite;
	...
	public StandardStyleComposite(Composite parent) {
	...
	public void createWidgets() throws Exception {
		...
		// Color Tab
		{
    		...
    		colorComposite = new StandardStyleColorComposite(tabFolder);
		...
		// Font Tab
		{
			...
			fontComposite = new StandardStyleFontComposite(tabFolder);
		...
		// Size Tab
		{
			...
			sizeComposite = new StandardStyleSizeComposite(tabFolder);
...
//					directory = new File("/Users/sacha/tmp"); // comment-in for debug
		    		...
		    		if ( ! applicationLessFile.exists()) {
		    			throw new RuntimeException(STANDARD_LESS_RESOURCES_FILE_NAME + " does not contain the expected file application.less.");
		    		}
...
//		}
	...
	protected void copyLessResources(File dir) throws ClassNotFoundException, IOException {
		...
		if (styleTemplateClass != null) {
			...
			if (StandardStyleProvider.STANDARD_STYLE.getStyleTemplateFileName().equals(pageLayout.getStyleTemplateFileName())) {
				inputStream = styleTemplateClass.getResourceAsStream(STANDARD_LESS_RESOURCES_FILE_NAME);
			}
			else if (StandardStyleProvider.CLASSIC_STYLE.getStyleTemplateFileName().equals(pageLayout.getStyleTemplateFileName())) {
