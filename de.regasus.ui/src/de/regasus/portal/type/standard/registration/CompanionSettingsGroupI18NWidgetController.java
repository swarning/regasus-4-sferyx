...
public class CompanionSettingsGroupI18NWidgetController implements I18NWidgetController<StandardRegistrationPortalConfig> {
	...
	public void createWidgets(Composite parent, ModifySupport modifySupport, String language) {
		...
		messageWidgetMap.put(language, builder.label( StandardRegistrationPortalConfig.COMPANION_WITHOUT_BOOKING_MESSAGE.getString() ).build());
	}
	...
	@Override
	public StandardRegistrationPortalConfig getEntity() {
		return portalConfig;
	}


	@Override
	public void setEntity(StandardRegistrationPortalConfig standardRegistrationPortalConfig) {
	...
	private void syncWidgetsToEntity() {
		if (portalConfig != null) {
			SWTHelper.syncExecDisplayThread(new Runnable() {
				...
				public void run() {
					try {
						setLanguageStringToTextWidget(portalConfig.getCompanionWithoutBookingMessage(), messageWidgetMap);
					}
					catch (Exception e) {
	...
	public void syncEntityToWidgets() {
		if (portalConfig != null) {
			portalConfig.setCompanionWithoutBookingMessage(buildLanguageString(messageWidgetMap));
		}
	}
