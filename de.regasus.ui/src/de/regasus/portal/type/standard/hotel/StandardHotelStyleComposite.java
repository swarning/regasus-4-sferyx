...
/**
 * Special Composite for StandardGroupStyleProvider.
 * However, it uses sub-Composites from de.regasus.portal.type.standard.
 */
public class StandardHotelStyleComposite extends Composite implements StyleParameterComposite {

	protected static final String STANDARD_HOTEL_LESS_RESOURCES_FILE_NAME = "standardhotel-less-resources.zip";
	protected static final String CLASSIC_HOTEL_LESS_RESOURCES_FILE_NAME = "classichotel-less-resources.zip";
	...
	// Composites from de.regasus.portal.type.standard
	private StandardHotelStyleColorComposite colorComposite;
	private StandardHotelStyleFontComposite fontComposite;
	private StandardHotelStyleSizeComposite sizeComposite;
	...
	public StandardHotelStyleComposite(Composite parent) {
	...
	public void createWidgets() throws Exception {
		...
		// Color Tab
		{
    		...
    		colorComposite = new StandardHotelStyleColorComposite(tabFolder);
		...
		// Font Tab
		{
			...
			fontComposite = new StandardHotelStyleFontComposite(tabFolder);
		...
		// Size Tab
		{
			...
			sizeComposite = new StandardHotelStyleSizeComposite(tabFolder);
...
//					directory = new File("/Users/sacha/tmp"); // comment-in for debug
		    		...
		    		if ( ! applicationLessFile.exists()) {
		    			throw new RuntimeException(STANDARD_HOTEL_LESS_RESOURCES_FILE_NAME + " does not contain the expected file application.less.");
		    		}
		    		File cssFile = new File(tmpDir, "standardhotel.css");
...
//		}
	...
	protected void copyLessResources(File dir) throws ClassNotFoundException, IOException {
		...
		if (styleTemplateClass != null) {
			...
			if (StandardHotelStyleProvider.STANDARD_HOTEL_STYLE.getStyleTemplateFileName().equals(pageLayout.getStyleTemplateFileName())) {
				inputStream = styleTemplateClass.getResourceAsStream(STANDARD_HOTEL_LESS_RESOURCES_FILE_NAME); 
			}
			else if (StandardHotelStyleProvider.CLASSIC_HOTEL_STYLE.getStyleTemplateFileName().equals(pageLayout.getStyleTemplateFileName())) {
