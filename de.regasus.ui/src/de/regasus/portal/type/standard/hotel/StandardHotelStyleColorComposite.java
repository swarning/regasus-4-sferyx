...
public class StandardHotelStyleColorComposite extends Composite {
	...
	private static final String NAVBAR_COLOR_KEY = "color-navbar";
	private static final RGB NAVBAR_COLOR_STANDARD_DEFAULT = new RGB(0x4e, 0x53, 0x53);
	private static final RGB NAVBAR_COLOR_CLASSIC_DEFAULT = new RGB(0x4e, 0x53, 0x53);
	...
	private ColorChooser navbarColorChooser;
	...
	public StandardHotelStyleColorComposite(Composite parent) {
	...
	public void createWidgets(Composite parent) {
		...
		// leftComposite
		{
    		...
    		// navbar color
    		{
    			Label label = new Label(leftComposite, SWT.NONE);
    			labelGridDataFactory.applyTo(label);
    			label.setText(I18N.PageLayoutEditor_StandardStyleComposite_NavbarColor);

    			navbarColorChooser = new ColorChooser(leftComposite);
    			colorChooserGridDataFactory.applyTo(navbarColorChooser);
    			navbarColorChooser.addModifyListener(modifySupport);
    		}
	...
	private boolean isStandard() {
		...
		try {
			if (pageLayout != null) {
				result = "standardhotel".equals(FileHelper.getNameWithoutExtension(pageLayout.getStyleTemplateFileName()));
			}
		} 
	...
	private void setStandardColor() {
		...
		navbarColorChooser.setDefaultColor(
			standard ? NAVBAR_COLOR_STANDARD_DEFAULT : NAVBAR_COLOR_CLASSIC_DEFAULT
		);
			...
			SWTHelper.syncExecDisplayThread(new Runnable() {
				...
				public void run() {
					try {
						...
						navbarColorChooser.setColorAsString( styleParameters.getProperty(NAVBAR_COLOR_KEY));
	...
	public void syncEntityToWidgets() {
		if (pageLayout != null) {
			...
			styleParameters.setProperty(NAVBAR_COLOR_KEY, navbarColorChooser.getColorAsString() );
