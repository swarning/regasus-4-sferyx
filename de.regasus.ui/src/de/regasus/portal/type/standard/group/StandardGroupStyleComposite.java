 ...
 */
public class StandardGroupStyleComposite extends Composite implements StyleParameterComposite {

	protected static final String STANDARD_GROUP_LESS_RESOURCES_FILE_NAME = "standardgroup-less-resources.zip";
	protected static final String CLASSIC_GROUP_LESS_RESOURCES_FILE_NAME = "classicgroup-less-resources.zip";
	...
	private StandardGroupStyleColorComposite colorComposite;
	private StandardGroupStyleFontComposite fontComposite;
	private StandardGroupStyleSizeComposite sizeComposite;
	...
	public StandardGroupStyleComposite(Composite parent) {
	...
	public void createWidgets() throws Exception {
		...
		// Color Tab
		{
    		...
    		colorComposite = new StandardGroupStyleColorComposite(tabFolder);
		...
		// Font Tab
		{
			...
			fontComposite = new StandardGroupStyleFontComposite(tabFolder);
		...
		// Size Tab
		{
			...
			sizeComposite = new StandardGroupStyleSizeComposite(tabFolder);
...
//					directory = new File("/Users/sacha/tmp"); // comment-in for debug
		    		...
		    		if ( ! applicationLessFile.exists()) {
		    			throw new RuntimeException(STANDARD_GROUP_LESS_RESOURCES_FILE_NAME + " does not contain the expected file application.less.");
		    		}
...
//		}
	...
	protected void copyLessResources(File dir) throws ClassNotFoundException, IOException {
		...
		if (styleTemplateClass != null) {
			...
			if (StandardGroupStyleProvider.STANDARD_GROUP_STYLE.getStyleTemplateFileName().equals(pageLayout.getStyleTemplateFileName())) {
				inputStream = styleTemplateClass.getResourceAsStream(STANDARD_GROUP_LESS_RESOURCES_FILE_NAME); 
			}
			else if (StandardGroupStyleProvider.CLASSIC_GROUP_STYLE.getStyleTemplateFileName().equals(pageLayout.getStyleTemplateFileName())) {
