...
public class OpenAmountLabelProvider extends ColumnLabelProvider {


	private AccountancyHelper accountancyHelper;


	public OpenAmountLabelProvider(AccountancyHelper accountancyHelper) {
	...
	public String getText(Object element) {
		if (element instanceof InvoiceVO) {
			...
			return invoice.getAmountOpenAsCurrencyAmount().format(false, true);
		}
		else if (element instanceof InvoicePositionVO) {
			InvoicePositionVO position = (InvoicePositionVO) element;
			return position.getAmountOpenAsCurrencyAmount().format(false, true);
		}
		else if (element instanceof String) {
			String currency = (String) element;
			CurrencyAmount currencyAmount = accountancyHelper.getOpenInvoiceAmountByCurrency(currency);
