...
/**
 * A handler for the command "Cancellation-by-Participant". 
 */
public class CancelParticipantByParticipantCommandHandler extends AbstractHandler {
		...
		try {
			...
			if (notEmpty(participantList)) {
				...
				if (participantList.size() == 1) {
					...
					question = I18N.CancelOneParticipantByParticipant_Question.replaceFirst("<name>", name);
				}
				else {
					String count = String.valueOf(participantList.size());
					question = I18N.CancelManyParticipantsByParticipant_Question.replaceFirst("<count>", count);
				}
	...
	private void executeInChunks (
		...
		ChunkExecutor<IParticipant> chunkExecutor = new ChunkExecutor<IParticipant>() {
			...
			protected void executeChunk(List<IParticipant> chunkList) throws Exception {
				...
				ParticipantModel.getInstance().cancelParticipantsByParticipant(
					chunkList, 
					withSubParticipants
				);
		...
		String operationMessage = I18N.CancelParticipantsByParticipant;
		...
		SWTHelper.syncExecDisplayThread(new Runnable() {
			public void run() {
				try {
					...
					String message = I18N.CancelParticipantsByParticipant_FinalMessage;
