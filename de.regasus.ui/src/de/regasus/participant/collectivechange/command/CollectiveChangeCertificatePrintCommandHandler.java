...
import de.regasus.participant.collectivechange.dialog.CollectiveChangeCertificatePrintDialog;
import de.regasus.participant.editor.ParticipantEditor;
import de.regasus.ui.Activator;

public class CollectiveChangeCertificatePrintCommandHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		try {
			...
			if (notEmpty(participants)) {
				...
				if (allEditorSaved) {
					// open Dialog
					CollectiveChangeCertificatePrintDialog dialog = new CollectiveChangeCertificatePrintDialog(
						shell,
						participantPKs.size()
					);
					dialog.create();
					...
					if (returnCode == TitleAreaDialog.OK) {
						Date certificatePrintDate = dialog.getCertificatePrintDate();
	...
	private void executeInChunks (
		...
		final Date certificatePrintDate
	) {
		final int[] counter = {0};
		...
		ChunkExecutor<Long> chunkExecutor = new ChunkExecutor<Long>() {
			...
			protected void executeChunk(List<Long> chunkList) throws Exception {
				ParticipantModel.getInstance().updateCertificatePrintDate(certificatePrintDate, chunkList);

				// add number of Participants that have just been changed certificate print
		...
		String operationMessage = I18N.CollectiveChangeCertificatePrintDialog_Title;
		...
		SWTHelper.syncExecDisplayThread(new Runnable() {
			...
			public void run() {
				try{
					...
					String message;
					if (certificatePrintDate == null) {
						message = I18N.CollectiveChangeCertificatePrint_FinalMessageDelete;
					}
					else {
						message = I18N.CollectiveChangeCertificatePrint_FinalMessageSet;
					}
