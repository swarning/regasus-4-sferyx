 ...
 */
public class HotelContingentSearchView extends AbstractView implements EventIdProvider {
	public static final String ID = "HotelContingentSearchView";

	private HotelContingentSearchViewPreference preference;

	// Widgets
	private EventCombo eventCombo;

	private HotelContingentSearchComposite searchComposite;
	...
	public HotelContingentSearchView() {
		preference = HotelContingentSearchViewPreference.getInstance();
	}
	...
	public void createWidgets(Composite parent) throws Exception {
		if (isVisible()) {
			...
			// within the HotelContingentSearchComposite
			searchComposite = new HotelContingentSearchComposite(
				parent,
			...
			// make the Table the SelectionProvider - for possible commands that might work on selected hotel contigents
	...
	private void initEventDependentWidgets() {
		...
		arrivalDateComposite.setLocalDate( TypeHelper.toLocalDate(arrival) );
		departureDateComposite.setLocalDate( TypeHelper.toLocalDate(departure) );
	...
	private void initializeDoubleClickAction() {
		...
		tableViewer.addDoubleClickListener(new IDoubleClickListener() {
			...
			public void doubleClick(DoubleClickEvent event) {
				List<HotelContingentVO> selectedHotelContingentVOs = SelectionHelper.getSelection(tableViewer, HotelContingentVO.class);
				for (HotelContingentVO hotelContingentVO : selectedHotelContingentVOs) {
					Long hotelContingentID = hotelContingentVO.getPrimaryKey();
					try {
						getSite().getPage().openEditor(new HotelContingentEditorInput(hotelContingentID), HotelContingentEditor.ID);
					}
					catch (PartInitException e) {
						RegasusErrorHandler.handleError(Activator.PLUGIN_ID.getClass().getName(), HotelContingentSearchView.class.getName(), e);
					}
				}
			}
		});
