...
public class DeleteProgrammeOfferingHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (!currentSelection.isEmpty()) {
			boolean deleteOK = MessageDialog.openConfirm(
				...
				UtilI18N.Question,
				I18N.DeleteProgrammeOfferingAction_Confirmation
			);
					...
					public void run() {
						...
						while (iterator.hasNext()) {
							...
							if (object instanceof ProgrammeOfferingTreeNode) {
								ProgrammeOfferingTreeNode node = (ProgrammeOfferingTreeNode) object;
								ProgrammeOfferingVO programmeOfferingVO = node.getValue();
								try {
									ProgrammeOfferingModel.getInstance().delete(programmeOfferingVO.getPK());
								}
								catch (Exception e) {
